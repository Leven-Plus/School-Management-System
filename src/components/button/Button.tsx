...
const Button = ({ name, onCLick, disabled, style = "primary", extras }: ButtonProp) => {
    return (
        <>
            <button onClick={onCLick} disabled={disabled} className={`${extras} ${buttonStyle[style]} ${disabled ? "opacity-50 pointer-events-none" : ""}`}>
                {name}
            </button>
